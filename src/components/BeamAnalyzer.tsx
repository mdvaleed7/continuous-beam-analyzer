...
export default function BeamAnalyzer() {
    // 1. State definitions
    const [nSpans, setNSpans] = useState(1);
    ...
    // BUG-B1: ref on the content wrapper so a ResizeObserver can trigger canvas
    // redraws when the container (not the window) changes width.
    ...
    useEffect(() => {
        ...
        // BUG-B1 FIX: redraw when the canvas parent resizes. Previously the canvas
        // kept its old pixel width on window/container resize (inline style.width
        // overrode the CSS width:100%), causing horizontal overflow. A single
        // ResizeObserver on the content wrapper covers all three canvases.
