...
function analyzeBeam(cfg: any): any {
    ...
    const nodeDeflections = reactions.map((_, i) => ({
...
export function computeSpanDeflection(
    spanInfo: any,
    nodeDefl: { v_L: number; theta_L: number; v_R: number; theta_R: number },
    w_ref_phys: number,
    L_ref_phys: number,
    EI_ref_phys: number,
    nPts: number = 60,
): { x: number; deflection_mm: number }[] {
    const alpha = spanInfo.alpha.fl();
    const L = alpha * L_ref_phys; // Physical span length in m
    const EI = EI_ref_phys; // kN·m²
    
    // Scale factors from normalized to physical
    const scaleFactor = (w_ref_phys * Math.pow(L_ref_phys, 4)) / EI_ref_phys;
    const v_L_phys = nodeDefl.v_L * scaleFactor;
    const v_R_phys = nodeDefl.v_R * scaleFactor;
    
    const thetaScale = (w_ref_phys * Math.pow(L_ref_phys, 3)) / EI_ref_phys;
    const theta_L_phys = nodeDefl.theta_L * thetaScale;
    const theta_R_phys = nodeDefl.theta_R * thetaScale;
    
    const wl = spanInfo.wL_frac.fl() * w_ref_phys;
    const wr = spanInfo.wR_frac.fl() * w_ref_phys;
    
    const pts: { x: number; deflection_mm: number }[] = [];
    for (let i = 0; i <= nPts; i++) {
        const xi = i / nPts;
        const x = xi * L;
        
        const N1 =  1 - 3*xi*xi + 2*xi*xi*xi;
        const N2 =  xi*(1-xi)*(1-xi);
        const N3 =  3*xi*xi - 2*xi*xi*xi;
        const N4 =  xi*xi*(xi-1);
        
        const v_homog = N1 * v_L_phys + N2 * theta_L_phys * L
                      + N3 * v_R_phys + N4 * theta_R_phys * L;
        
        const term = (x * x * (L - x) * (L - x)) / (120 * L * EI);
        const v_part = term * (wl * (3 * L - 2 * x) + wr * (2 * L + x));
        
        pts.push({ x, deflection_mm: (v_homog + v_part) * 1000 });
    }
    return pts;
}
...
export { analyzeBeam, F, toFrac, ZERO, ONE,  alphaLabel,  formatCoeffTerm,  };
