...
import { REPORT_CSS, FIT_FORMULAS_SCRIPT, inputTable } from './reportCss';
...
// IS 456 Cl. 23.2(b): the optimizer only accepts a camber-assisted design when
// the demanded upward camber is at most this cap (mm). Surfaced in the
// deflection report so a checker can audit camber compliance without re-running
// the optimizer. Must match the `reqCamber <= 20` gate in slabEngine.optimizeSlab.
const CAMBER_CAP_MM = 20;
        ...
        <div class="section-body">
            ...
            <div style="display: flex; gap: 20px;">
                ...
                <div style="flex: 1; min-width: 0;">
                    ...
                    ${dfl.camber > 0
                        ? `<div style="margin: 4px 0; padding: 6px 10px; background: #f0fdf4; border-left: 3px solid #10b981; font-size: 12px; color: #475569; text-align: left;">
                            <strong>Camber audit (IS 456 Cl. 23.2(b)):</strong>
                            this design was accepted only after applying an upward camber of
                            ${dfl.camber.toFixed(1)} mm, which is
                            ${dfl.camber <= CAMBER_CAP_MM
                                ? `within the ${CAMBER_CAP_MM} mm cap (${dfl.camber.toFixed(1)} mm &le; ${CAMBER_CAP_MM} mm) &mdash; <span style="color:#10b981;font-weight:bold;">compliant</span>.`
                                : `<span style="color:#ef4444;font-weight:bold;">above the ${CAMBER_CAP_MM} mm cap (${dfl.camber.toFixed(1)} mm &gt; ${CAMBER_CAP_MM} mm) &mdash; revise</span>.`}
                            Reviewers can audit camber compliance here without re-running the optimizer.
                        </div>`
                        : ''
                    }
                    ${dfl.camber > 0
...
function shearSection(r: any, directions: {label: string, dir: any}[]): string {
    ...
    const shearBlocks = directions.map(({label, dir}) => {
        ...
        const vuFormula = hasBeta
            ? kx(`V_u = \\beta \\times w_u \\times l_x = ${dir.beta} \\times ${r.wFactored} \\times ${r.Lx} = ${dir.Vu} \\text{ kN/m}`)
            : kx(`V_u = ${dir.Vu} \\text{ kN}`);
        const vuCritical = dir.Vu_critical != null
            ? kx(`V_{u,crit} = V_u - w_u \\times d = ${dir.Vu_critical} \\text{ kN/m} \\quad (\\text{at } d = ${dir.d_eff} \\text{ mm from face})`)
            : '';
        return `
        <div style="margin-bottom: 15px; padding-bottom: 15px; border-bottom: 1px dashed #e2e8f0;">
            ...
            <div style="display: flex; gap: 20px;">
                <div style="flex: 1; min-width: 0;">
                    ${vuFormula}
                    ${vuCritical}
                    ${kx(`\\tau_v = \\frac{V_{u,crit}}{b \\cdot d} = ${dir.tau_v} \\text{ N/mm}^2`)}
    ...
    return `
    <div class="section-box avoid-break">
        <div class="section-header">Shear Check &mdash; IS 456 Cl. 40 ${tableRef ? `(${tableRef})` : ''}</div>
        <div class="section-body">
            <p style="margin-top:0; color:#475569; font-size:13px;">
                <strong>Variables:</strong><br/>
