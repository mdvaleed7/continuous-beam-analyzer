...
import { REPORT_CSS, FIT_FORMULAS_SCRIPT, calcRow, inputTable } from './reportCss';
...
function flexBlock(label: string, Mu: number, AstReq: number, AstMin: number, pt: number, governs: string, status: string, fck: number, fy: number, d: number, d_req_bm: number, depthStatus: string): string {
    // R = coeff * fck. Using a simple reverse calc from d_req_bm to show R in the report
    const R = (Mu * 1e6) / (1000 * Math.pow(d_req_bm || 1, 2));

    return `
        <div style="margin-bottom: 20px;">
            <h4 style="margin: 0 0 10px 0; color: #334155;">${label}</h4>
            ${kx(`M_u = ${Mu.toFixed(2)} \\text{ kN}\\cdot\\text{m}`)}
            ${kx(`d_{req} = \\sqrt{\\frac{M_u \\times 10^6}{R_u b}} = ${d_req_bm.toFixed(2)} \\text{ mm}`)}
            ${kx(`d_{prov} = ${d} \\text{ mm}`)}
            <div style="margin: 4px 0 10px 0; font-size: 13px; font-weight: bold; color: ${depthStatus === 'FAIL' ? '#ef4444' : '#10b981'};">
                Depth Adequacy: ${kxInline(`d_{prov} ${depthStatus === 'OK' ? '\\ge' : '<'} d_{req}`)} &rarr; ${depthStatus}
            </div>
            ${governs === 'design' ? 
    ...
    return `
        ...
        <div class="section-box avoid-break">
            ...
            <div class="section-body">
                ...
                <div style="display: flex; gap: 20px;">
                    <div style="flex: 1; min-width: 0;">
                        ${flexBlock('X-Direction (parallel L)', r.flexureX.Mu, r.flexureX.Ast_req, r.flexureX.Ast_min, r.flexureX.pt, r.flexureX.governs, r.flexureX.status, r.fck, r.fy, r.dEffX, r.flexureX.d_req_bm, r.flexureX.depthStatus)}
                    </div>
                    <div style="flex: 1; min-width: 0;">
                        ${flexBlock('Z-Direction (parallel B)', r.flexureZ.Mu, r.flexureZ.Ast_req, r.flexureZ.Ast_min, r.flexureZ.pt, r.flexureZ.governs, r.flexureZ.status, r.fck, r.fy, r.dEffZ, r.flexureZ.d_req_bm, r.flexureZ.depthStatus)}
                    </div>
                </div>
            </div>
        </div>
    ...
    return `
        ...
        <div class="section-box avoid-break">
            ...
            <div class="section-body">
                ...
                <div style="display: flex; gap: 20px;">
                    <div style="flex: 1; min-width: 0;">
                        ${flexBlock('X-Direction', r.flexureX.Mu, r.flexureX.Ast_req, r.flexureX.Ast_min, r.flexureX.pt, r.flexureX.governs, r.flexureX.status, r.fck, r.fy, r.dEffX, r.flexureX.d_req_bm, r.flexureX.depthStatus)}
                    </div>
                    <div style="flex: 1; min-width: 0;">
                        ${flexBlock('Z-Direction', r.flexureZ.Mu, r.flexureZ.Ast_req, r.flexureZ.Ast_min, r.flexureZ.pt, r.flexureZ.governs, r.flexureZ.status, r.fck, r.fy, r.dEffZ, r.flexureZ.d_req_bm, r.flexureZ.depthStatus)}
                    </div>
                </div>
            </div>
        </div>
