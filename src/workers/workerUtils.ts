...
const DEFAULT_COST_RATIO = 80;
const sanitize = <T>(r: T): T => JSON.parse(JSON.stringify(r));
...
interface DoneMessage<R = unknown> {
    type: 'done';
    result: R;
}
...
type WorkerOutbound<R = unknown> = ProgressMessage | DoneMessage<R> | ErrorMessage;
